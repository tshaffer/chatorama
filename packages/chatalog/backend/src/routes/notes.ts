...
notesRouter.post('/:noteId/index-linked-pages-text', async (req, res, next) => {
  try {
    ...
    if (!snapshots.length) {
      return res.status(400).json({ error: 'snapshots is required' });
    }
    if (snapshots.length > MAX_URLS_PER_REQUEST) {
      return res.status(400).json({ error: `Too many urls (max ${MAX_URLS_PER_REQUEST})` });
    }
    ...
    for (const snap of snapshots) {
      const urlRaw = String(snap?.url ?? '').trim();
      if (!urlRaw) {
      ...
      try {
        ...
        const extractedTextRaw = String(snap?.extractedText ?? '');
        ...
        const title = snap?.title ? String(snap.title) : undefined;
        const excerpt = snap?.excerpt ? String(snap.excerpt) : undefined;
